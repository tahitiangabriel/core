...
public class HomePage extends WicketExamplePage {
	...
	public HomePage() {
		...
		map.add(new ClickListener() {
			...
			@Override
			protected void onClick(final AjaxRequestTarget target, final GLatLng gLatLng) {
				if (gLatLng != null) {
					if (infoWindow != null) {
						infoWindow.close();
					}
					// Create the infoPanel
					final Component c = new InfoPanel(INFOPANEL, i);
					i++;
					...
					target.addComponent(hiddenContainersInfoPanel);
		...
		lbInfoWindow.add(new AjaxEventBehavior("onclick") {
			...
			protected void onEvent(final AjaxRequestTarget target) {
				final GInfoWindow tmpInfoWindow =
				        new GInfoWindow(new GLatLng(37.5 * (0.9995 + Math.random() / 1000), -122.1
				                * (0.9995 + Math.random() / 1000)), "Opened via button");
